...
// 🖼️ Custom Shail Logo Intro Screen
async function shailIntro() {
  if (document.getElementById('shail-intro')) return;
  ...
  img.src = 'asset/Kings and Pigs.png'; // <-- make sure this file exists
  img.alt = 'King and Pigs';
  img.style = `
    max-width: 60vw;
    max-height: 60vh;
  `.trim();
  ...
  await new Promise(resolve => setTimeout(resolve, 1000)); // show for 1s
  div.style.opacity = '0';

  await new Promise(resolve => setTimeout(resolve, 1000)); // fade out
  div.remove();
}
